...
def get_all_protein_resids(pdb_file):
    try:
        prot = Molecule(pdb_file, type="pdb")
    except:
...
def show_probes(probefile):
    try:
        probes = Molecule(probefile, type="pdb")
    ...
    if writeprobes:
        ...
        with open(probefile, "w") as f:
            for i, site in enumerate(sites):
                print(site)
                f.write(
                    f"HETATM  {i+1:3} ZN    ZN A  1    {site[0][0]: 8.3f}{site[0][1]: 8.3f}{site[0][2]: 8.3f}  {site[1]:.2f}  0.0            {id} \n"
                )
    return sites
...
def write_probefile(sites, identities, probefile):
    file_content = ""
    with open(probefile, "w") as f:
        for i, (site,p) in enumerate(sites):
            ...
            probe_line = f"HETATM  {i+1:3}{id}   {id} A{i+1:3}    {site[0]: 8.3f}{site[1]: 8.3f}{site[2]: 8.3f}  {p:.2f}  0.0            {id}  \n"
            f.write(
                probe_line
            )
